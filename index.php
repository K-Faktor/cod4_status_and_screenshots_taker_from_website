		...
		foreach($xml_data as $player){
			...
			foreach($player->attributes() as $player_data => $data){
				...
				if(in_array($player_data, $only_need)){
					...
					if($player_data == 'Ping' ){
						echo '<td>  <form method="POST"><button type="submit" class="btn btn-primary btn-block" data-somestringvalue-text="Screenshot Requested" autocomplete="off" name="player_CID" value="'.$player_CID.'"><i class="fa fa-camera"></i> Take Screenshot</button></form></td>';
					}
				}
			}
			echo '</tr>';
